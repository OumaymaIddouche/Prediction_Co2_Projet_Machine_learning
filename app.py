...
def load_resources():
    global loaded_models, mlp_scaler, mlp_columns, feature_columns_csv
    global marques, carburants_options, transmissions_options
    ...
    # 2. SCALER (CRITIQUE POUR LE MLP)
    if os.path.exists('scaler.pkl'):
        try:
            with open('scaler.pkl', 'rb') as f:
                mlp_scaler = pickle.load(f)
            print("✓ Scaler MLP chargé (Indispensable pour des prédictions correctes).")
        except Exception as e:
            print(f"✗ Erreur Scaler: {e}")
    else:
        print("⚠️ ATTENTION: scaler.pkl manquant. Le MLP donnera des résultats faux.")

    # 3. Colonnes MLP
    if os.path.exists('mlp_cols.pkl'):
        with open('mlp_cols.pkl', 'rb') as f:
            mlp_columns = pickle.load(f)
...
def predict():
    try:
        ...
        for model_name, model_obj in loaded_models.items():
            try:
                # Si c'est le MLP, on utilise le SCALER et ses colonnes spécifiques
                if 'MLP' in model_name and mlp_scaler and mlp_columns:
                    # A. Aligner les données dans l'ordre exact de l'entraînement MLP
                    features_mlp = [input_data.get(col, 0.0) for col in mlp_columns]
                    
                    # B. NORMALISER les données
                    features_scaled = mlp_scaler.transform(np.array([features_mlp]))
                    
                    # C. Prédire sur les données normalisées
                    pred = model_obj.predict(features_scaled)[0]
                    
                # Si c'est RF ou XGBoost (qui n'ont pas utilisé de scaler)
                else:
                    features_rf = [input_data.get(col, 0.0) for col in feature_columns_csv]
                    pred = model_obj.predict(np.array([features_rf]))[0]
